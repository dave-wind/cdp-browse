...
The `cdp_sdk` package is pip-installed. Run Python via Bash:
...
If `import cdp_sdk` fails, install it first: `pip install cdp-sdk`
...
client.find_session()
page = Page(client)
page.navigate(url, wait_for_js="document.readyState === 'complete'", timeout=10)
time.sleep(2)  # settle for dynamic content
