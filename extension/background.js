// background.js - Cookie + CDP Browse
chrome.runtime.onInstalled.addListener(() => {
  console.log("CDP Browse installed");
...
async function handleCookies(msg, sender) {
  try {
    ...
    const merged = [...all];
    for (const c of part) {
      if (!merged.some((x) => x.name === c.name && x.domain === c.domain))
        merged.push(c);
    }
  ...
  try {
    ...
    try {
      ...
      if (res === null || res === undefined) {
        console.log(
          "[TMWD-WS] executeScript returned null/undefined, treating as CSP issue",
        );
        res = {
          ok: false,
          error: {
    ...
    if (res && !res.ok && res.csp) {
      ...
      try {
        await chrome.debugger.attach({ tabId }, "1.3");
        const cdpRes = await chrome.debugger.sendCommand(
          { tabId },
          "Runtime.evaluate",
          {
        ...
        if (cdpRes.exceptionDetails) {
          const desc =
            cdpRes.exceptionDetails.exception?.description || "CDP Error";
          res = {
            ok: false,
            error: { name: "Error", message: desc, stack: desc },
          };
        } else {
        ...
        res = {
          ...
          error: {
            name: "Error",
            message: "CDP fallback failed: " + cdpErr.message,
            stack: "",
          },
        };
      }
    ...
    try {
      ...
      if (data.id && data.code) {
        ...
        if (typeof code === "object" && code !== null && code.cmd) {
          ...
          if (code.tabId === undefined && data.tabId !== undefined)
